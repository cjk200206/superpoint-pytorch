...
class Event(data.Dataset):
    ...
    def __init__(self, export=False, transform=None, task='train', **config):
        ...
        # get files
        base_path = DATA_PATH +'/'+'Event/'+ task
        type_paths = sorted(os.listdir(base_path))
        image_paths = []
        names = []
        
        for type_path in type_paths:  #airplane,bxxx,cxxx
            number_paths = sorted(os.listdir(base_path +'/'+type_path)) #imgs_0001 -> 0xxx
            for number_path in number_paths:
                temp_number_path = Path(base_path +'/'+type_path+'/'+number_path)
                temp_paths = sorted(list(temp_number_path.iterdir()))
                # if config['truncate']:
                #     image_paths = image_paths[:config['truncate']]
                
                # temp_names = [p.stem for p in temp_paths]
                temp_names = [str(type_path)+'_'+str(number_path)+'_'+p.stem for p in temp_paths]
                temp_image_paths = [str(p) for p in temp_paths]
